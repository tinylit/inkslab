...
namespace Inkslab.Tests
{
    ...
    public class LRUTests
    {
        ...
        public void DisposableStruct_DisposesOnlyActuallyEvictedValue()
        {
            ...
            var lru = new Lru<int, DisposableValue>(1, new ConstantHashComparer(), key => new DisposableValue(key));

            Assert.Equal(1, lru.Get(1).Value);
            Assert.Equal(1, lru.Get(1).Value);
            Assert.Equal(0, DisposableValue.DisposeCount);

            Assert.Equal(2, lru.Get(2).Value);
        ...
        {
            DisposableValue.Reset();
            var lru = new Lru<int, DisposableValue>(1, _ => throw new InvalidOperationException("Factory failed."));

            Assert.Throws<InvalidOperationException>(() => lru.Get(1));
            Assert.Equal(0, DisposableValue.DisposeCount);
        }
