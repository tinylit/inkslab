...
namespace Inkslab.Collections
{
    ...
    public class AsynchronousDictionary<TKey, TValue> : IReadOnlyDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable
    {
        ...
        {
            if (storage.TryGetValue(key, out TValue stackValue))
            {
                return stackValue;
            }

            using (asynchronousLock.Acquire())
            {
        ...
        public TValue GetOrAdd(TKey key, Func<TKey, TValue> valueFactory)
        {
            ...
            using (asynchronousLock.Acquire())
            {
                return storage.GetOrAdd(key, valueFactory);
            }
        }
        ...
        public TValue AddOrUpdate(TKey key, TValue addValue, Func<TKey, TValue, TValue> updateValueFactory)
        {
            using (asynchronousLock.Acquire())
            {
                return storage.AddOrUpdate(key, addValue, updateValueFactory);
            }
        }
        ...
        public TValue AddOrUpdate(TKey key, Func<TKey, TValue> addValueFactory, Func<TKey, TValue, TValue> updateValueFactory)
        {
            using (asynchronousLock.Acquire())
            {
                return storage.AddOrUpdate(key, addValueFactory, updateValueFactory);
            }
        }
